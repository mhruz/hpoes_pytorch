...
def points2range_ver2(points3D, com3D, cube=None, dsize=None):
    ...
    im = np.zeros(dsize[:2], np.float32)
    val = points3D[:,2]
    val = val[idx]
...
def saveCrops(argv, masksize=(200,200)):
    ...
    for i, (labelline, bbline) in enumerate(zip(labelsfilelist, bbcomfilelist)):
        ...
        try:
            dpt, M, com = hd.cropArea3D(com=com, size=cube, dsize=dsize[:2], docom=docom)
        except UserWarning:
...
def saveH5(argv):
    ...
    dsize= np.array(argv['dsize'], np.int32) #(224, 224) traning image resolution or (88,88,88) for voxels
    ...
    for json in jsonfilelist:
        ...
        if argv['H5labelformat'] == '3Dprojection':
            ...
            if (argv['H5datagroup'] == 'real_voxels'):
                ...
                imgD = np.zeros((dsize[0], dsize[1], dsize[2]), dtype='bool')
                imgD[ind] = True
                #visualisation(imgD)
...
def showH5(argv):
    ...
    for i in range(0,N):
        ...
        if argv['H5datagroup'] == 'real_voxels':
            ...
            plotlabels((labels + 1.0) * dsize[0]/2.)
            mlab.show()
